...
if len(sys.argv) > 2:
    ...
    firmware_zip_file_path = '_build/dfu_app_hrs_s132_nrf52_sdk12.3.0.zip'
...
def dfu_create_command_object(firmware_data, DFU_control_point):
    create = b"\x01"
    command_object = b"\x01"
    size_object = get_size(firmware_data)
...
# Sending file
def dfu_send_file(file_path, DFU_packet, mtu):
...
def dfu_request_crc(DFU_control_point):
    calculate_crc_code = b"\x03"
    dfu_packet = calculate_crc_code
    DFU_control_point.value = dfu_packet
    sleep(3)
    print("---------")
...
def dfu_execute_sent_file(DFU_control_point):
    ...
    dfu_packet = excetute_code
    DFU_control_point.value = excetute_code
    sleep(3)
    print("---------")
...
# Create Data Object
def dfu_create_data_object(firmware_bin, DFU_control_point):
    create = b"\x01"
    data_object = b"\x02"
    size_object = get_size(firmware_bin)
...
dfu_send_file(firmware_dat, DFU_packet, mtu)

print("Request to calculate checksum \n")
dfu_request_crc(DFU_control_point)

print("Execute the sent file (the app.dat file)\n")
...
print("Select last commnad and create Data Object\n")
dfu_select_last_command_and_create_data_object(DFU_control_point)

print("Create Data Object\n")
dfu_create_data_object(firmware_bin, DFU_control_point)

print("Sending the firmware bin file\n")
dfu_send_file(firmware_bin, DFU_packet, mtu)

print("Request to calculate checksum \n")
dfu_request_crc(DFU_control_point)

print("Execute the sent file (the app.bin file)\n")
dfu_execute_sent_file(DFU_control_point)
